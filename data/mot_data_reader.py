...
#        return len(self.detection_group_keys)
        return self.detection_group_keys[len(self.detection_group_keys)-1]

    def get_detection_by_index(self, index):
#        if index > len(self.detection_group_keys) or self.detection_group_keys.count(index)==0:
        if index > self.detection_group_keys[len(self.detection_group_keys)-1] or self.detection_group_keys.count(index)==0:
            return None
        return self.detection_group.get_group(index).values

    def get_image_by_index(self, index):
#        if index > len(self.detection_group_keys):
        if index > self.detection_group_keys[len(self.detection_group_keys)-1]:
            return None
